...
export async function createTask(
  ...
  const db = await getDatabase();
  await db.runAsync(
    'INSERT INTO tasks (title, category, recurrence, recurrence_days, xp) VALUES (?, ?, ?, ?, ?)',
    title,
    category,
    recurrence,
...
export async function archiveTask(id: number): Promise<void> {
  const db = await getDatabase();
  await db.runAsync("UPDATE tasks SET status = 'archived', completed_at = datetime('now') WHERE id = ?", id);
}
